...
class Vec3 {
     ...
     */
    distance(vec3) {
        const difference = this.subtract(vec3);
        return Math.sqrt(difference.x * difference.x + difference.y * difference.y + difference.z * difference.z);
    }

    /**
     * Whether this vector's components are equal to another vector's components.
     * @param {Vec3} vec3 - The other vector to compare.
    ...
    mix(a, x) {
        if (a instanceof Vec4) {
            if (x instanceof Vec4) {
                return vec4(
                    Math.lerp(this.x, a.x, x.x),
                    Math.lerp(this.y, a.y, x.y),
                    Math.lerp(this.z, a.z, x.z),
                    Math.lerp(this.w, a.w, x.w)
                );
            } else if (typeof x == "number") {
                return vec4(
                    Math.lerp(this.x, a.x, x),
                    Math.lerp(this.y, a.y, x),
                    Math.lerp(this.z, a.z, x),
                    Math.lerp(this.w, a.w, x)
                );
            } else {
            ...
            if (x instanceof Vec3) {
                return vec4(
                    Math.lerp(this.x, a, x.x),
                    Math.lerp(this.y, a, x.y),
                    Math.lerp(this.z, a, x.z),
                    Math.lerp(this.w, a, x.w)
                );
            } else if (typeof x == "number") {
                return vec4(
                    Math.lerp(this.x, a, x),
                    Math.lerp(this.y, a, x),
                    Math.lerp(this.z, a, x),
                    Math.lerp(this.w, a, x)
                );
            } else {
                throw new Error("b must be number or Vec4");
            }
