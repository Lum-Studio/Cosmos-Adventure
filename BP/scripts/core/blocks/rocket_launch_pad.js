...
world.beforeEvents.worldInitialize.subscribe(({ itemComponentRegistry }) => {
	itemComponentRegistry.registerCustomComponent("cosmos:rocket", {
		onUseOn({ block, source: player, usedOnBlockPermutation: pad, itemStack: item }) {
			if (block.typeId != "cosmos:rocket_launch_pad" ||
				!pad.getState("cosmos:center") ||
				item.typeId != "cosmos:rocket_tier_1_item" ||
				player.dimension.getEntities({ location: block.center(), maxDistance: 1 }).length
			) return
			...
			if (player.getGameMode() != "creative") player.hand(item.decrementStack())
		}
	})
})
