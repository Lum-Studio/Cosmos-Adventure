...
function grappleProjectileStop(location, projectile){
	...
	let generalProjectile = owner.dimension.spawnEntity('cosmos:generalGrapple', owner.location);
...
world.beforeEvents.worldInitialize.subscribe(({itemComponentRegistry}) => {
    itemComponentRegistry.registerCustomComponent("cosmos:grapple", {
        onCompleteUse(data){
            let visualProjectile = data.source.dimension.spawnEntity('cosmos:visuallGrapple', data.source.location);
...
world.afterEvents.projectileHitBlock.subscribe((data) => {
    if(data.projectile.typeId != 'cosmos:visuallGrapple' && data.projectile.typeId != 'cosmos:generalGrapple') return;
    if(data.projectile.typeId === 'cosmos:visuallGrapple') grappleProjectileStop(data.location, data.projectile);
    if(data.projectile.typeId === 'cosmos:generalGrapple') try {world.getEntity(data.projectile.getDynamicProperty('generalGrappleProjectile')).triggerEvent("cosmos:despawn"), data.projectile.triggerEvent("cosmos:despawn")} catch(error) {null}
});