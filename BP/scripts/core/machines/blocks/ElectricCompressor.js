...
const data = {
	...
	onTick(entity, block) {
		...
		if (energy > 0 && progress < 200 && recipe && (one_has_space(64) || two_has_space(64))) {
			progress = progress + 5;
			energy -= Math.min(50, energy);
		}

		if ((energy == 0 || (recipe === undefined || (!one_has_space(64) && !two_has_space(64)))) && progress > 0) progress = progress - 5
		if (progress == 200) {
			block.dimension.playSound("random.anvil_land", entity.location)
			progress = 0
			let min = 64;
			for (let i = 0; i < 9; i++) {
				if(items[i] && items[i].amount < min) min = items[i].amount
			}
			if(min == 1 || (!one_has_space(63) && !two_has_space(63))){
			for (let i = 0; i < 9; i++) {
		...
		if(!compare_lists(first_values, [energy, progress]) || !container.getItem(EnergyDisplay)){
			...
			container.add_ui_display(ProgressDisplay, '', Math.ceil((progress / 200) * 52))
