 ...
 * Custom Gravitational Computational Engine for Scripting V2 for Minecraft Bedrock
...
class Gravity {
  ...
  applyKnockbackWithDamage(targetEntity, vector, power) {
    ...
    if (typeof targetEntity.applyKnockback === "function") {
      // Normalize the original horizontal direction and multiply by the old horizontal strength value (with resistance factor)
      const horizontalDir = { x: Number(vector.x), z: Number(vector.z) };
      const magnitude = Math.sqrt(horizontalDir.x * horizontalDir.x + horizontalDir.z * horizontalDir.z);
      if (magnitude > 0) {
        horizontalDir.x /= magnitude;
        horizontalDir.z /= magnitude;
      }
      const oldHorizontalStrength = Number(vector.hzPower) * resistanceFactor;
      const newHorizontal = { x: horizontalDir.x * oldHorizontalStrength, z: horizontalDir.z * oldHorizontalStrength };
      targetEntity.applyKnockback(newHorizontal, Number(adjustedPower.y));
    }
  }
  ...
  applyJump() {
    ...
    const executeJumpStep = (step) => {
      ...
      if (typeof entity.applyKnockback === "function") {
        // New API: horizontal vector and vertical strength separated.
        entity.applyKnockback({ x: 0, z: 0 }, perTickImpulse * progress);
      }
...
async function applyGravityEffects(entity, vector, currentFall, gravityValue, gravity) {
    ...
    if (blockBelow && blockBelow.typeId === "minecraft:slime_block") {
      ...
      if (currentFall < bounceThreshold) {
        ...
        if (typeof entity.applyKnockback === "function") {
          entity.applyKnockback({ x: 0, z: 0 }, bounceImpulse);
        }
    ...
    if (typeof entity.applyKnockback === "function") {
      ...
      const newHorizontal = { x: horizontalDir.x * Number(vector.hzPower), z: horizontalDir.z * Number(vector.hzPower) };
      entity.applyKnockback(newHorizontal, Number(knockbackPower));
    }
 ...
 */
system.worldLoad.entityHitEntity.subscribe(event => {
